#!/usr/bin/env python3
"""
Fetch latest dyslexia & reading research papers from PubMed E-utilities API.
Targets core dyslexia journals and covers major reading science topics.
"""
...
def build_query(days: int = 7, max_journals: int = 15) -> str:
    ...
    topic_part = (
        '(dyslexia[tiab] OR "developmental dyslexia"[tiab] '
        'OR "reading disorder"[tiab] OR "reading disability"[tiab] '
        'OR "specific reading disorder"[tiab])'
    )

    return f"({journal_part}) AND {date_part} AND {topic_part}"
...
def main():
    parser = argparse.ArgumentParser(description="Fetch dyslexia papers from PubMed")
    ...
    print(
        f"[INFO] Searching PubMed for dyslexia papers from last {args.days} days...",
        file=sys.stderr,
    )
